...
function ServicesForCompaniesAndPersons({
  ...
  return (
    <div className=" p-6 flex flex-col gap-3 rounded-3xl border border-neutral-300 bg-neutral-100 shadow-shadow3 max-sm:p-5 max-sm:pl-4">
      <div className=" flex gap-2 items-center">
        <div className="[&>svg]:size-6 [&>svg_path]:fill-neutral-700">
          {type === "company" && <FinanceIcon />}
          {type === "person" && <UserCertificationIcon />}
        </div>
        <p className=" text-neutral-700 font-bold leading-7 text-xl max-sm:text-lg">
          {title}
        </p>
      </div>
      <ul className=" text-neutral-500 text-base font-medium leading-7 list-disc list-inside pr-3 max-sm:text-sm max-sm:leading-6">
        {items.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
